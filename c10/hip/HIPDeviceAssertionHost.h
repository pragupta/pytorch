...
#ifdef USE_HIP
#define TORCH_USE_HIP_DSA
#endif

/// Number of assertion failure messages we can store. If this is too small
/// threads will fail silently.
constexpr int C10_HIP_DSA_ASSERTION_COUNT = 10;
constexpr int C10_HIP_DSA_MAX_STR_LEN = 512;
...
struct DeviceAssertionData {
  ...
  char assertion_msg[C10_HIP_DSA_MAX_STR_LEN];
  /// File the assertion was in
  char filename[C10_HIP_DSA_MAX_STR_LEN];
  /// Name of the function the assertion was in
  char function_name[C10_HIP_DSA_MAX_STR_LEN];
...
struct DeviceAssertionsData {
  ...
  DeviceAssertionData assertions[C10_HIP_DSA_ASSERTION_COUNT];
};
...
struct HIPKernelLaunchInfo {
  ...
  /// HIPKernelLaunchRegistry::gather_launch_stacktrace is True
...
/// occurred HIPKernelLaunchRegistry is used as a singleton
class C10_HIP_API HIPKernelLaunchRegistry {
 private:
 ...
 public:
  ...
  std::
      pair<std::vector<DeviceAssertionsData>, std::vector<HIPKernelLaunchInfo>>
      snapshot() const;
  ...
  static HIPKernelLaunchRegistry& get_singleton_ref();
...
#define TORCH_DSA_KERNEL_ARGS                                              \
  [[maybe_unused]] c10::hip::DeviceAssertionsData *const assertions_data, \
