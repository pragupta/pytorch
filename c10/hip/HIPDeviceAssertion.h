...
static __device__ void dstrcpy(char* dst, const char* src) {
  ...
  // isn't longer than `C10_HIP_DSA_MAX_STR_LEN-1`
  while (*src != '\0' && i++ < C10_HIP_DSA_MAX_STR_LEN - 1) {
    *dst++ = *src++;
  }
  *dst = '\0';
}
...
static __device__ void dsa_add_new_assertion_failure(
  ...
  if (nid >= C10_HIP_DSA_ASSERTION_COUNT) {
...
#define CUDA_KERNEL_ASSERT2(condition)                                   \
  do {                                                                   \
    if (C10_UNLIKELY(!(condition))) {                                    \
      ...
      c10::hip::dsa_add_new_assertion_failure(                          \
          assertions_data,                                               \
