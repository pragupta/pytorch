...
void CachingAllocatorConfig::parseArgs(const char* env) {
  ...
  for (size_t i = 0; i < config.size(); i++) {
    ...
    } else if (config[i] == "release_lock_on_hipmalloc") {
      used_native_specific_option = true;
      consumeToken(config, ++i, ':');
      ++i;
      TORCH_CHECK(
...
class DeviceCachingAllocator {
 private:
  ...
  ska::flat_hash_map<hipStream_t, MempoolId_t> stream_to_pool_map;
 ...
 public:
  ...
  Block* malloc(int device, size_t orig_size, hipStream_t stream) {
    ...
    if (!block_found) {
      ...
      TORCH_CHECK_WITH(
          ...
          " fragmentation.  See documentation for Memory Management and PYTORCH_HIP_ALLOC_CONF",
          "");
    }
  ...
  void beginAllocateStreamToPool(hipStream_t stream, MempoolId_t mempool_id) {
    std::lock_guard<std::recursive_mutex> lock(mutex);
    captures_underway++;
  ...
  void endAllocateStreamToPool(hipStream_t stream) {
    std::lock_guard<std::recursive_mutex> lock(mutex);
    captures_underway--;
 ...
 private:
  ...
  bool alloc_block(
    ...
    if (set_fraction &&
      ...
      if (CachingAllocatorConfig::release_lock_on_hipmalloc()) {
