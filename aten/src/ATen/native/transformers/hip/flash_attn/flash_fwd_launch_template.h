...
#include <ATen/native/transformers/hip/flash_attn/flash_fwd_kernel.h>
...
void run_mha_fwd_hdim256(Flash_fwd_params &params, hipStream_t stream) {
    ...
    hipError_t status_ = hipDeviceGetAttribute(
        &max_smem_per_sm, hipDeviceAttributeMaxSharedMemoryPerMultiprocessor, device);
    status_ = hipDeviceGetAttribute(
        &max_smem_per_block, cudaDevAttrMaxSharedMemoryPerBlockOptin, device);
