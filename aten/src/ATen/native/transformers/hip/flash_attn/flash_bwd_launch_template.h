...
#include <ATen/native/transformers/hip/flash_attn/static_switch.h>
#include <ATen/native/transformers/hip/flash_attn/flash.h>
#include <ATen/native/transformers/hip/flash_attn/flash_bwd_kernel.h>
...
void run_flash_bwd_seqk_parallel(Flash_bwd_params &params, hipStream_t stream, const bool configure) {
    ...
    if (Kernel_traits::kSmemdQSize >= 48 * 1024)  {
        C10_HIP_CHECK(hipFuncSetAttribute(
            kernel_dq, hipFuncAttributeMaxDynamicSharedMemorySize, Kernel_traits::kSmemdQSize));
    }
...
template<typename Kernel_traits, bool Is_dropout>
void run_flash_bwd_seqq_parallel(Flash_bwd_params &params, hipStream_t stream, const bool configure) {
    const int num_n_block = (params.seqlen_k + Kernel_traits::kBlockN - 1) / Kernel_traits::kBlockN;
    ...
    if (Kernel_traits::kSmemKVSize >= 48 * 1024)  {
        C10_HIP_CHECK(hipFuncSetAttribute(
            kernel_dkv, hipFuncAttributeMaxDynamicSharedMemorySize, Kernel_traits::kSmemKVSize));
    }
