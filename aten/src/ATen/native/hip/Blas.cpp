...
Tensor& addmm_out_cuda_impl(Tensor& result, const Tensor& self, const Tensor& mat1, const Tensor& mat2, const Scalar& beta, const Scalar& alpha, Activation activation=Activation::None) {
  ...
  if (&result != &self) {
#if defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 11040 && !defined(_MSC_VER)
    // Strangely, if mat2 has only 1 row or column, we get
    // HIPBLAS_STATUS_INVALID_VALUE error from cublasLtMatmulAlgoGetHeuristic.
...
#if !defined(USE_ROCM) && !defined(_MSC_VER) && defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 11070
...
#else
#if !defined(USE_ROCM) && !defined(_MSC_VER) && defined(TORCH_HIP_VERSION)
