...
struct DropoutState {
  ...
  std::mutex mutex;
#if !defined(USE_ROCM)
  // hipStreamGetCaptureInfo will never give back a capture id of 0, so 0 can serve
  ...
  void lock() {
    ...
    if (event) {
      ...
      AT_CUDA_CHECK(hipStreamGetCaptureInfo(hip::getCurrentHIPStreamMasqueradingAsCUDA(),
                                             &status,
...
Tensor try_get_weight_buf(
      ...
      miopenRNNMode_t mode, c10::SymInt hidden_size, c10::SymInt proj_size, int64_t num_layers, bool bidirectional) {
