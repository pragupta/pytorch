...
CaptureId_t capture_sequence_id() {
  // id starts at 1:
  // Ensures uuid count starts at 1. 0 is reserved to mean "not set by hipStreamGetCaptureInfo".
...
void CUDAGraph::capture_begin(MempoolId_t pool/*=0*/, hipStreamCaptureMode capture_mode) {
  ...
  c10::hip::HIPCachingAllocator::beginAllocateStreamToPool(capture_dev_, capture_stream_, mempool_id_);
  ...
  AT_CUDA_CHECK(hipStreamGetCaptureInfo(stream, &status, nullptr));
...
void CUDAGraph::capture_end() {
  ...
  c10::hip::HIPCachingAllocator::endAllocateStreamToPool(capture_dev_, capture_stream_);
  ...
  AT_CUDA_CHECK(hipGraphGetNodes(graph_, NULL, &numCUDAGraphNodes));
...
#if (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 11030)|| (defined(USE_ROCM) && ROCM_VERSION >= 50600)
  if (_cuda_graphs_debug) {
    TORCH_WARN("DEBUG: calling debug_dump()");
    if (has_graph_) {
      TORCH_WARN("DEBUG: calling hipGraphDebugDotPrint() with ", debug_path);
      C10_HIP_CHECK_WARN(hipGraphDebugDotPrint(graph_, debug_path.c_str(), 1<<10)); // most verbose output
      AT_CUDA_CHECK(hipGraphDestroy(graph_));
    }
  } else {
