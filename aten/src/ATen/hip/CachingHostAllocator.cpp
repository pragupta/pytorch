...
struct Block {
  ...
  std::unordered_set<at::hip::HIPStreamMasqueradingAsCUDA> streams_;
};
 ...
 * free list, and otherwise we create a new block with hipHostMalloc. In the
...
class CUDAHostAllocator {
 public:
  std::pair<void*, void*> allocate(size_t size) {
    ...
    void* ptr = nullptr;
    C10_HIP_CHECK(hipHostMalloc(
        &ptr, c10::llvm::PowerOf2Ceil(size), hipHostMallocDefault));
  ...
  bool record_event(void* ptr, void* ctx, at::hip::HIPStreamMasqueradingAsCUDA stream) {
    auto* block = reinterpret_cast<Block*>(ctx);
  ...
  void empty_cache() {
    ...
    for (auto* block : blocks_to_remove) {
      ...
      AT_CUDA_CHECK(hipHostFree(block->ptr_));
      delete block;
    }
  }
 ...
 private:
  void process_events() {
    while (true) {
      // Avoid calling hipEventDestroy while holding a mutex, so move
