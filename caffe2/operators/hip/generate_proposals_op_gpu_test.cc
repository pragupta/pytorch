...
TEST(GenerateProposalsTest, TestRealDownSampledGPU) {
  ...
  auto& rois_gpu = rois_blob->Get<TensorHIP>();
  Tensor rois{CPU};
  ...
  auto& rois_probs_gpu = rois_probs_blob->Get<TensorHIP>();
...
#if defined(CV_hipLibraryMajorVersion) && (CV_hipLibraryMajorVersion >= 3)
TEST(GenerateProposalsTest, TestRealDownSampledRotatedAngle0GPU) {
  ...
  AddInput<HIPContext>(
      ...
      &ws);
  AddInput<HIPContext>(vector<int64_t>{img_count, 3}, im_info, "im_info", &ws);
  AddInput<HIPContext>(vector<int64_t>{A, box_dim}, anchors, "anchors", &ws);
...
#endif // CV_hipLibraryMajorVersion >= 3
