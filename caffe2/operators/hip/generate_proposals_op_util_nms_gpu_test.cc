...
#include "caffe2/operators/hip/generate_proposals_op_util_nms_gpu.h"
...
TEST(UtilsNMSTest, TestNMSGPU) {
  ...
  HIPContext hip_context(option);

  Tensor dev_sorted_boxes{HIP};
  Tensor dev_scores{HIP};
  Tensor dev_boxes_valid_flags{HIP};
  Tensor dev_list{HIP};
  Tensor dev_delete_mask{HIP};
  Tensor host_delete_mask{CPU};
  Tensor dev_list_nitems{HIP};
  ...
  for (int itest = 0; itest < input_thresh.size(); ++itest) {
    ...
    hip_context.FinishDeviceComputation();
...
TEST(UtilsNMSTest, TestPerfNMS) {
  ...
  HIP_CHECK(hipStreamSynchronize(hip_context.hip_stream()));
